...
<script setup>
  ...
  /**
   * This function is used when you need to access route-specific information like query parameters, route params, name, path, matched components, etc.
   */
  const route = useRoute();

  const article = ref({})
  const loadArticleData = (articleID) => {
    axios.get(`http://localhost:3004/articles/${articleID}`).then(response => {
  ...
  // watch(()=>route.params.articleID, async newId=> loadArticleData(newId))
